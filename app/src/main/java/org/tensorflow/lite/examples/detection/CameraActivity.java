...
import org.tensorflow.lite.examples.detection.barcode.ApiClient;
import org.tensorflow.lite.examples.detection.barcode.BarcodeListFragment;
import org.tensorflow.lite.examples.detection.barcode.BarcodeRequest;
import org.tensorflow.lite.examples.detection.barcode.BarcodeResponse;
import org.tensorflow.lite.examples.detection.barcode.RestInterfaces;
...
public abstract class CameraActivity extends AppCompatActivity implements OnImageAvailableListener, Camera.PreviewCallback, CompoundButton.OnCheckedChangeListener, View.OnClickListener {
  ...
  private Button showBarcode,sendBarcode;
  protected TextView barcodeCount;
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    barcodeCount = findViewById(R.id.count);
    showBarcode = findViewById(R.id.showBarcode);
    sendBarcode = findViewById(R.id.sendBarcode);
    restInterfaces = ApiClient.getClient().create(RestInterfaces.class);
    ...
    showBarcode.setOnClickListener(this);
    sendBarcode.setOnClickListener(this);
  }

  public void updateCount(){
    this.runOnUiThread(() -> {
      barcodeCount.setText(getBarcodeCount());
    });
  }
  ...
  public static void clearBarcode(){
      barcodeList.clear();
      cameraActivity.updateCount();
      BarcodeListFragment.updateAdapter();
  }
  ...
  public static String getBarcodeCount(){
    return  String.valueOf(barcodeList.size());
  }

  public static void setToolbarVisible(boolean status){
      if(!status){
          cameraActivity.toolbar.setVisibility(View.GONE);
      }else{
          cameraActivity.toolbar.setVisibility(View.VISIBLE);
      }
  }
  ...
  public void onClick(View v) {
    if (v.getId() == R.id.showBarcode) {
        Fragment f = getFragmentManager().findFragmentById(R.id.container);
        if(f instanceof  BarcodeListFragment){
            clearBarcode();
        }else{
            BarcodeListFragment barcodeFragment = new BarcodeListFragment();
            setToolbarVisible(false);
            showBarcode.setText("Barkodları Temizle");
            this.changeFragment(barcodeFragment,"fragment");
        }
    }else if(v.getId() == R.id.sendBarcode){
      ...
      if(barcodeList.size()>0){
        ...
        Call<BarcodeResponse> r = restInterfaces.sendBarcodeList(req);
        r.enqueue(new Callback<BarcodeResponse>() {
          @Override
          public void onResponse(Call<BarcodeResponse> call, Response<BarcodeResponse> response) {
            BarcodeResponse res = response.body();
            if(response.code()==200){
              clearBarcode();
              cameraActivity.runOnUiThread(() -> {
                Toast.makeText(cameraActivity,"Barkodlar Gönderildi",Toast.LENGTH_LONG).show();
              });
            }else{
              cameraActivity.runOnUiThread(() -> {
                Toast.makeText(cameraActivity,"HATA!",Toast.LENGTH_LONG).show();
              });
            }
          }
          @Override
  ...
  public void onBackPressed() {
      Fragment f = getFragmentManager().findFragmentById(R.id.container);
      if(f instanceof BarcodeListFragment){
          showBarcode.setText("Barkodları Göster");
